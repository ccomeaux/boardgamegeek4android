 ...
 * the {@link android.widget.SimpleCursorAdapter.CursorToStringConverter} and the
 * {@link android.widget.FilterQueryProvider} interfaces to get control over the filtering process. You can refer to
 * {@link #convertToString(android.database.Cursor)} and {@link #runQueryOnBackgroundThread(CharSequence)} for more
 * information.
 */
public class SimpleDragSortCursorAdapter extends ResourceDragSortCursorAdapter {
	/**
	 * A list of columns containing the data to bind to the UI. This field should be made private, so it is hidden from
	 * the SDK. {@hide}
	 */
	protected int[] mFrom;
	/**
	 * A list of View ids representing the views to which the data must be bound. This field should be made private, so
	 ...
	 * 
	 * @deprecated This option is discouraged, as it results in Cursor queries being performed on the application's UI
	 *             thread and thus can cause poor responsiveness or even Application Not Responding errors. As an
	 *             alternative, use {@link android.app.LoaderManager} with a {@link android.content.CursorLoader}.
	 */
	@Deprecated
	 ...
	 * @param layout
	 *            resource identifier of a layout file that defines the views for this list item. The layout file should
	 *            include at least those named views defined in "to"
	 * @param c
	 *            The database cursor. Can be null if the cursor is not available yet.
	 * @param from
	 *            A list of column names representing the data to bind to the UI. Can be null if the cursor is not
	 *            available yet.
	 * @param to
	 *            The views that should display column in the "from" parameter. These should all be TextViews. The first
	...
	/**
	 * Create a map from an array of strings to an array of column-id integers in cursor c. If c is null, the array will
	 * be discarded.
	 * 
	 * @param c
	...
	/**
	 * This class can be used by external clients of SimpleCursorAdapter to define how the Cursor should be converted to
	 * a String.
	 * 
	 * @see android.widget.CursorAdapter#convertToString(android.database.Cursor)
	 */
