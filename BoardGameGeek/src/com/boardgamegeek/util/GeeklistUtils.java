...
public class GeeklistUtils {
    ...
    public static String convertBoardGameGeekXmlText(String text)
	{
        ...
        while (matcher.find()) {
            matcher.appendReplacement(result, "<div style=\"display:inline\"><img src=\"" + BoardGameGeekConstants.BGG_STATIC_IMAGE + "pic" + matcher.group(1) + "_t.jpg\"/></div>");
        }
        ...
        while (matcher.find()) {
            ...
            if (anchorText == null || anchorText.isEmpty()) {
                matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_GEEKLIST + matcher.group(1) + "\">geeklist "+matcher.group(1)+"</a>");
            }
            else {
                matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_GEEKLIST + matcher.group(1) + "\">"+anchorText+"</a>");
            }
        }
        matcher.appendTail(result);
        text = result.toString();
        ...
        while (matcher.find()) {
            ...
            if (thingText == null || thingText.isEmpty()) {
                try {
                    ...
                    matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_BOARDGAME + gameId + "\">boardgame " + gameId + "</a>");
                    //Game game = boardGameCache.getGame(gameId);
                    //matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_BOARDGAME + gameId + "\">" + game.getName() + "</a>");
                }
                catch (Exception e) {
                    matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_BOARDGAME + matcher.group(1) + "\">boardgame " + matcher.group(1) + "</a>");
                }
            }
            else {
                matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_BOARDGAME + matcher.group(1) + "\">"+thingText+"</a>");
            }
        }
        matcher.appendTail(result);
        text = result.toString();
        ...
        while (matcher.find()) {
            String anchorText = matcher.group(2);
            if (anchorText == null || anchorText.isEmpty()) {
                matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_USER + matcher.group(1) + "\">" + matcher.group(1) + "</a>");
            }
            else {
                matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_USER + matcher.group(1) + "\">" + anchorText + "</a>");
            }
        }
        matcher.appendTail(result);
        text = result.toString();
        ...
        while (matcher.find()) {
            ...
            if (threadText == null || threadText.isEmpty()) {
                matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_THREAD + matcher.group(1) + "\">thread " + matcher.group(1) + "</a>");
            }
            else {
                matcher.appendReplacement(result, "<a href=\"" + BoardGameGeekConstants.BGG_THREAD + matcher.group(1) + "\">" + threadText + "</a>");
            }
        }
        matcher.appendTail(result);
        text = result.toString();
