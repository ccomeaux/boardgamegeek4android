...
public abstract class ImageWorker {
	 ...
	 * if an {@link ImageCache} has been set using {@link ImageWorker#addImageCache}. If the image is found in the
	...
	/**
	 * Adds an {@link ImageCache} to this worker in the background (to prevent disk access on UI thread) using default
	 * cache parameters.
	 * 
	...
	public void addImageCache(FragmentActivity fragmentActivity) {
		addImageCache(fragmentActivity.getSupportFragmentManager(), new ImageCache.ImageCacheParams(fragmentActivity));
	}

	/**
	 * Sets the {@link ImageCache} object to use with this ImageWorker. Usually you will not need to call this directly,
	 * instead use {@link ImageWorker#addImageCache} which will create and add the {@link ImageCache} object in a
	 * background thread (to ensure no disk access on the main/UI thread).
	 * 
	 * @param imageCache
	 */
	...
	/**
	 * Setting this to true will signal the working tasks to exit processing at the next chance. This helps finish up
	 * pending work when the activity is no longer in the foreground and completing the tasks is no longer useful.
	 * 
