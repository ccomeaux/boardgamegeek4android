...
interface PlayDao2 {
    ...
    @Query("SELECT * FROM play_players JOIN plays ON plays._id = play_players._play_id WHERE object_id = :gameId AND (delete_timestamp=0 OR delete_timestamp IS NULL)")
    suspend fun loadPlayersForGame(gameId: Int): List<PlayPlayerEntity>
