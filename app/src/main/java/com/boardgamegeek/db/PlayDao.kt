...
interface PlayDao {
    ...
    @Query("SELECT MAX(own) AS owned, plays.*, games.subtype, games.game_rank FROM plays LEFT JOIN games ON plays.object_id = games.game_id LEFT JOIN collection ON plays.object_id = collection.game_id GROUP BY plays.play_id ORDER BY date DESC, play_id DESC")
    ...
    @Query("SELECT play_players.*, plays.quantity, plays.no_win_stats AS noWinStats, plays.incomplete, plays.date, users.avatar_url AS avatarUrl, users.first_name AS firstName, users.last_name AS lastName FROM play_players JOIN plays ON plays._id = play_players._play_id LEFT JOIN users ON users.username = play_players.user_name WHERE user_name = :username AND (delete_timestamp=0 OR delete_timestamp IS NULL)")
    ...
    @Query("SELECT play_players.*, plays.quantity, plays.no_win_stats AS noWinStats, plays.incomplete, plays.date, users.avatar_url AS avatarUrl, users.first_name AS firstName, users.last_name AS lastName FROM play_players JOIN plays ON plays._id = play_players._play_id LEFT JOIN users ON users.username = play_players.user_name WHERE name = :name AND (user_name = '' OR user_name IS NULL)")
    ...
    @Query("SELECT play_players.*, plays.quantity, plays.no_win_stats AS noWinStats, plays.incomplete, plays.date, users.avatar_url AS avatarUrl, users.first_name AS firstName, users.last_name AS lastName FROM play_players JOIN plays ON plays._id = play_players._play_id LEFT JOIN users ON users.username = play_players.user_name WHERE object_id = :gameId AND (delete_timestamp=0 OR delete_timestamp IS NULL)")
    ...
    @Query("SELECT play_players.*, plays.quantity, plays.no_win_stats AS noWinStats, plays.incomplete, plays.date, users.avatar_url AS avatarUrl, users.first_name AS firstName, users.last_name AS lastName FROM play_players JOIN plays ON plays._id = play_players._play_id LEFT JOIN users ON users.username = play_players.user_name WHERE delete_timestamp=0 OR delete_timestamp IS NULL")
    fun loadPlayersFlow(): Flow<List<PlayerWithPlayEntity>>

    @Query("SELECT play_players.*, plays.quantity, plays.no_win_stats AS noWinStats, plays.incomplete, plays.date, users.avatar_url AS avatarUrl, users.first_name AS firstName, users.last_name AS lastName FROM play_players JOIN plays ON plays._id = play_players._play_id LEFT JOIN users ON users.username = play_players.user_name WHERE location = :location AND (delete_timestamp=0 OR delete_timestamp IS NULL)")
