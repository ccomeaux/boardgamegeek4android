...
class TopGameRepository {
    ...
    suspend fun findTopGames(): List<TopGameEntity> = withContext(Dispatchers.IO) {
        var rank = 1
        val doc = Jsoup
        ...
        gameElements.map { element ->
            ...
            val yearPublishedText =
                gameNameElement?.child(1)?.text().orEmpty().trimStart('(').trimEnd(')').toIntOrNull() ?: TopGameEntity.YEAR_UNKNOWN
