...
import com.boardgamegeek.extensions.launchTaskWithResult
import com.boardgamegeek.provider.BggContract
import com.boardgamegeek.provider.BggContract.Collection
import com.boardgamegeek.util.ResolverUtils
import org.greenrobot.eventbus.EventBus

abstract class UpdateCollectionItemTask(context: Context?, protected val gameId: Int, protected val collectionId: Int, protected var internalId: Long) {
    @SuppressLint("StaticFieldLeak")
    private val context: Context? = context?.applicationContext

    fun execute() {
        launchTaskWithResult(
            backgroundWork = {
                if (context == null) return@launchTaskWithResult false
