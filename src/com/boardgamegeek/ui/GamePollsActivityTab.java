...
import com.boardgamegeek.ui.GameListsActivityTab.ChildItem;
...
public class GamePollsActivityTab extends ExpandableListActivity implements AsyncQueryListener {
		...
		try {
			if (token == TOKEN_POLLS) {
				...
				while (cursor.moveToNext()) {
					...
					int groupPosition = -1;
					if(poll.name.equals("suggested_numplayers")) {
						createPollGroup(poll.id, poll.title + ": " + players, poll.totalVotes, new ArrayList<PollResult>());
						groupPosition = mGroupData.size() - 1;
					} else {
						groupPosition = getPollGroupPosition(poll.id);
					}
					
					//TODO: default sort doesn't work correctly for player age due to it being saved as string
					mHandler.startQuery(TOKEN_POLL_RESULTS_RESULT, groupPosition, GamePollResults.buildPollResultsResultUri(id), 
	...
	class PollAdapter extends BaseExpandableListAdapter {
		...
		public PollAdapter() {
			inflater = (LayoutInflater) GamePollsActivityTab.this
		...
		public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView,
			...
			//TODO: ellipzie instead of putting level as text
			String text;
			if (result.level != 0) {
			...
			((TextView) v.findViewById(R.id.count)).setText("" + result.numberOfVotes + " / "
				+ group.get(COUNT));
